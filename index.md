...
$\mathrm{SMD} = \frac{ X_{c} - X_{c} }{ \sqrt{(s^{2}_{t} + s^{2}_{c})/2} }$ where $X_{t}$, $X_{c}$  denote the mean of that feature for the treatment and control group respectively. We will use absolute value of this number. $s_{t}$, $s_{c}$ denote the standard deviation of that feature for the treatment and control group respectively.
...
<iframe frameborder="no" border="0" marginwidth="-10%" marginheight="0" width="150%" height="300" allowfullscreen="true" src="assets/img/prop_dist_after.png"></iframe>
