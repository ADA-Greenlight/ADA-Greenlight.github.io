...
$\text{SMD} = \frac{\bar{X}_{c} - \bar{X}_{c}}{\sqrt{(s^{2}_{t} + s^{2}_{c})/2}}$ where $\bar{X}_{t}$, $\bar{X}_{c}$ denote the mean of that feature for the treatment and control group respectively. We will use absolute value of this number. $s_{t}$, $s_{c}$ denote the standard deviation of that feature for the treatment and control group respectively.
